...
def _handle_portstats_received(event):
      ...
      for f in event.stats:
          ...
          if int(f.port_no) < 65534: # used from hosts and switches interlinks, 34525
              ...
              estim_l_utility = estim_l_utility /1
              ...
              if estim_l_utility >= 0:
                  ...
                  if Links_ports [event.dpid, f.port_no] != []:
                     ...
                     #utility = [("%.3f" % i)[:-1] for i in utility]
                     utility = list(np.array(utility))
                     #g1 = [("%.3f" % i)[:-1] for i in g]
                     #print ('utilityn:',utility )
                     header = (Links_utilization.keys())
                     #print ('header:',header)
                     #print('header',header)
                     #headers = list(Links_utilization.keys())
                      ...
                      if counter/64 == int(counter/64):                  
                        with open('output.csv', 'a') as m: 
                           ...
                           if h_p == 0:
                               ...
                               m.write(datetime.now().strftime(format = '%Y-%m-%d %H:%M:%S,'))
                               m.write(str(headers))
                               m.write('\n')
                               h_p = 1  
                           ...
                           #m.write (str(format(utility, '.2f')[1 : -1]) )                    
                           m.write('\n')
...
# send OPF.mod mesage
  def _install (self, dl_src, dl_dst, path_port, p, buf = None):
    ...
    for i in path_port:
        ...
        #msg.hard_timeout = 16000 #20 minutes 
